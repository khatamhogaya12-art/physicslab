...
export default function App() {
  ...
  // Component positions
  const [compPos, setCompPos] = useState<Record<ComponentId, { x: number, y: number }>>({
    fg: { x: 30, y: 30 },
    v: { x: 230, y: 340 },
    bb: { x: 230, y: 50 },
    am: { x: 550, y: 30 }
  ...
  // Helper to check if two terminals are connected (directly or indirectly)
  // For simplicity in this lab, we'll use a direct connection check or a simple path finder
  ...
  const activeComponent = useMemo(() => {
    ...
    // The user's specific wiring scheme:
    // 1. BB_C is connected to one FG terminal
    // 2. One of BB_X, BB_Y, BB_Z is connected to one AM terminal
    // 3. The other AM terminal is connected to the other FG terminal
  ...
  const isVoltmeterConnected = useMemo(() => {
    // End-to-end connection: V+ and V- connected to different FG terminals
  ...
  return (
    <div className="min-h-screen bg-zinc-50 p-4 md:p-8 font-sans selection:bg-emerald-100">
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-zinc-200 pb-6">
          <div>
            ...
            <p className="text-zinc-500 mt-1 max-w-xl text-sm italic">
          ...
          <Button variant="outline" onClick={resetLab} className="group">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    className="h-7 text-[10px] uppercase font-bold text-zinc-400 hover:text-red-500"
                    ...
                    >
                      <div className="bg-zinc-50 rounded-lg p-3 text-[11px] font-mono text-zinc-600 space-y-1">
                        ...
                        <Button 
                          variant="outline" 
                          size="sm" 
                          className="w-full mt-4 bg-white hover:bg-zinc-100 text-zinc-900 border-zinc-200 gap-2"
                        ...
                        <Button variant="link" size="sm" className="p-0 h-auto mt-2 text-xs font-bold underline" onClick={resetLab}>
                          Reshuffle & Try Again
                        </Button>
                      </div>
                    </motion.div>
    ...
    >
      <div className="flex flex-col items-center">
        ...
        <button
          onClick={onClick}
          className={cn(
            "w-8 h-8 rounded-full border-4 transition-all duration-200 relative flex items-center justify-center shadow-xl",
            terminal.color,
            active 
